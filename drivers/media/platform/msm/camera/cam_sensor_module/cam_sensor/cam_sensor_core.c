    ...
    void *arg)
{
    ...
    case CAM_SENSOR_PROBE_CMD: {
        ...
        CAM_ERR(CAM_SENSOR, "power_upsetting: %p, poer_down_setting: %p", \
            power_info->power_setting, power_info->power_down_setting);
        ...
        CAM_ERR(CAM_SENSOR, "s_ctrl->sensordata->power_info.power_setting: %p", \
...
#ifdef VENDOR_EDIT
    	/*Added by Camera,2018/03/25  add for struct light module*/
    	if (s_ctrl->sensordata->slave_info.sensor_id != 0x6300) {
    		cam_sensor_release_resource(s_ctrl);
    		cam_sensor_release_stream_rsc(s_ctrl);
    	}
