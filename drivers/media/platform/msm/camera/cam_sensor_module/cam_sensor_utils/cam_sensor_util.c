	...
	uint16_t power_setting_size)
{
...
#ifdef VENDOR_EDIT
    /*Added by @Camera,2018/3/21  add for struct light module*/
	...
	for (i = 0; i < power_setting_size; i++) {
        ...
        CAM_ERR(CAM_SENSOR, "[%d], seq_type: %d", i, power_setting[i].seq_type);
        CAM_ERR(CAM_SENSOR, "VDIG: %d, VIO: %d, VANA: %d, VAF: %d, CUSTOM_REG1: %d, \
            CUSTOM_REG2: %d", SENSOR_VDIG, SENSOR_VIO, SENSOR_VANA, SENSOR_VAF, SENSOR_CUSTOM_REG1, \
            SENSOR_CUSTOM_REG2);
		...
		case SENSOR_CUSTOM_REG1:
			for (j = 0; j < num_vreg; j++) {
                CAM_ERR(CAM_SENSOR, "[%d], regltr_name: %s", j, soc_info->rgltr_name[j]);
	...
	while (tot_size < cmd_length) {
		if (cmm_hdr->cmd_type ==
			...
			ptr = (void *)
				(scr +
				sizeof(struct cam_cmd_unconditional_wait));
			CAM_DBG(CAM_SENSOR, "ptr: %p sizeof: %d Next: %p",
				scr, (int32_t)sizeof(
				struct cam_cmd_unconditional_wait), ptr);
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	gpio_num_info = ctrl->gpio_num_info;
	num_vreg = soc_info->num_rgltr;
	CAM_ERR(CAM_SENSOR, "gpio_num_info %p",  gpio_num_info);
#ifdef VENDOR_EDIT
    /*Added by @Camera,2018/3/21  add for struct light module*/
    if ((num_vreg < 0) || (num_vreg > CAM_SOC_MAX_REGULATOR)) {
        CAM_ERR(CAM_SENSOR, "failed: num_vreg %d", num_vreg);
