		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CHARGE_TECHNOLOGY:
			val->intval = oplus_vooc_get_charger_techonology();
			//charger_xlog_printk(CHG_LOG_CRTI, "Fast_charger_techonology:%d\n", val->intval);
			break;
		case POWER_SUPPLY_PROP_FAST_CHARGE:
			if(oplus_vooc_get_charger_techonology()){
				val->intval = oplus_chg_show_vooc_logo_ornot();
			}
			else{
				val->intval = 0;
			}
			//charger_xlog_printk(CHG_LOG_CRTI, "Fast_charger_type:%d\n", val->intval);
#ifdef CONFIG_OPLUS_CHARGER_MTK
			if (val->intval) {
