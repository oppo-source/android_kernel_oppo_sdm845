...
static bool stm8s_fw_check_lastline(struct oplus_vooc_chip *chip)
{
	...
	chg_err(" chip->fw_mcu_version = %x,chip->firmware_version = %x\n", chip->fw_mcu_version, chip->firmware_data[chip->fw_data_count - 4]);
	//modyfied for 17107 bad capacitance,only update when mcu_version < fw_version
	if (chip->fw_mcu_version < chip->firmware_data[chip->fw_data_count - 4])
		return FW_CHECK_FAIL;
	else
		return FW_CHECK_SUCCESS;
...
static void stm8s_fw_check_then_recover(struct oplus_vooc_chip *chip)
{
	int update_result = 0;
