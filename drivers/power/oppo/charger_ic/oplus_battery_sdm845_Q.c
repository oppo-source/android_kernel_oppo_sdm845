...
#ifdef OPLUS_FEATURE_CHG_BASIC
static struct pinctrl *uart_pinctrl = NULL;
static struct pinctrl_state *uart_pinctrl_to_outputlow = NULL;
static struct pinctrl_state *uart_pinctrl_to_default = NULL;
static bool outputlow_flag = false;
#endif
...
static int  send_clk_delay = 1000;
module_param_named(
	send_clk_delay, send_clk_delay, int, 0600);

static int  feedback_vbus_delay = 200;
module_param_named(
	feedback_vbus_delay, feedback_vbus_delay, int, 0600);

static int  read_vbus_interval = 100;
module_param_named(
	read_vbus_interval, read_vbus_interval, int, 0600);
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (is_project(OPPO_17107) && get_PCB_Version() == HW_VERSION__19) {//MP2
		if (apsd_result->bit != SDP_CHARGER_BIT && apsd_result->bit != CDP_CHARGER_BIT) {
			if (uart_pinctrl && uart_pinctrl_to_outputlow) {
#ifndef CONFIG_OPLUS_DAILY_BUILD
				pinctrl_select_state(uart_pinctrl, uart_pinctrl_to_outputlow);
#endif
				smbchg_set_chargerid_switch_val(1);
				outputlow_flag = true;
				chg_err("uart_pinctrl_to_outputlow\n");
			}
		}
	}
#endif /*OPLUS_FEATURE_CHG_BASIC*/
...
irqreturn_t smblib_handle_usb_source_change(int irq, void *data)
{
	...
	if ((stat & APSD_DTC_STATUS_DONE_BIT) && !chg->uusb_apsd_rerun_done) {
		...
		smblib_err(chg, "APSD_STATUS = 0x%02x, reg_value = %d\n",stat,reg_value);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen	PSW.BSP.CHG  2019-03-04  17127 for feedback vbus */
static int64_t vbus_feedback = -1;
static int oplus_feedback_vbus(void)
{
	int i, vbus;

	if (is_project(OPPO_17127) == false)
		return 0;
	...
	if (gpio_get_value(31) != 1) {
		chg_debug("charger exist, don't reset mcu\n");
		vbus_feedback = -1;
		return 0;
	}

	oplus_vooc_reset_mcu();
	mdelay(send_clk_delay);

	if (gpio_get_value(31) != 1) {
		chg_debug("charger exist, don't send clk\n");
		vbus_feedback = -1;
		return 0;
	}

	for (i = 0; i < 15; i ++) {
		oplus_vooc_set_ap_clk_low();
		usleep_range(2000, 2100);
		oplus_vooc_set_ap_clk_high();
	}

	mdelay(feedback_vbus_delay);
	if (gpio_get_value(31) != 1) {
		chg_debug("charger exist, don't feedback vbus\n");
		vbus_feedback = -1;
		return 0;
	}
	...
	vbus_feedback = 0;
	for (i = 0; i < 3; i ++) {
		vbus = qpnp_get_prop_charger_voltage_now();
		if (i == 0)
			vbus_feedback += (int64_t)vbus * 100000000;
		else if (i == 1)
			vbus_feedback += (int64_t)vbus * 10000;
		else
			vbus_feedback += (int64_t)vbus;
		mdelay(read_vbus_interval);
	}

	chg_debug("feedback vbus done! vbus_feedback = %lld\n", vbus_feedback);
	return 0;
}
#endif
...
void smbchg_set_chargerid_switch_val(int value)
{
	...
	if (value) {
#ifdef OPLUS_FEATURE_CHG_BASIC
		if (is_project(OPPO_17107) && get_PCB_Version() == HW_VERSION__19) {
			if (uart_pinctrl && uart_pinctrl_to_default && (outputlow_flag == true)) {
#ifndef CONFIG_OPLUS_DAILY_BUILD
				pinctrl_select_state(uart_pinctrl, uart_pinctrl_to_default);
#endif
				outputlow_flag = false;
				chg_err("uart_pinctrl_to_default\n");
			}
		}
#endif /*OPLUS_FEATURE_CHG_BASIC*/
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (usbtemp_volt == -1)//get adc fail and use default usbtemp
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen	PSW.BSP.CHG  2019-03-04  17127 for feedback vbus */
	case POWER_SUPPLY_PROP_VBUS_FEEDBACK:
		val->int64val = vbus_feedback;
		break;
#endif
	default:
#ifdef OPLUS_FEATURE_CHG_BASIC
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	uart_pinctrl = devm_pinctrl_get(oplus_chip->dev);
	if (IS_ERR_OR_NULL(uart_pinctrl)) {
		chg_err("No uart_pinctrl config specified!\n");
	} else {
		uart_pinctrl_to_outputlow =
				pinctrl_lookup_state(uart_pinctrl, "qupv3_se9_2uart_outputlow");
		uart_pinctrl_to_default =
				pinctrl_lookup_state(uart_pinctrl, "qupv3_se9_2uart_default");
		if (IS_ERR_OR_NULL(uart_pinctrl_to_outputlow) || IS_ERR_OR_NULL(uart_pinctrl_to_default)) {
			chg_err("No uart_pinctrl config specified!\n");
			uart_pinctrl = NULL;
			uart_pinctrl_to_outputlow = NULL;
			uart_pinctrl_to_default= NULL;
		}
	}
#endif /*OPLUS_FEATURE_CHG_BASIC*/
