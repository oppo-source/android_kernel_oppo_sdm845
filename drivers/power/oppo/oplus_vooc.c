...
static int oplus_vooc_convert_fast_chg_type_to_chargertechonology(int fast_chg_type);
...
void oplus_vooc_set_ap_clk_low(void)
{
        if (!g_vooc_chip) {
                return;
        } else {
                g_vooc_chip->vops->set_clock_active(g_vooc_chip);
        }
}
...
static int oplus_vooc_convert_fast_chg_type_to_chargertechonology(int fast_chg_type)
{
	...
	} else if (chip->support_11v3a_flashcharger_display){
...
int oplus_vooc_get_charger_techonology(void)
{
	struct oplus_vooc_chip *chip = g_vooc_chip;
	if (!chip) {
                return -1;
			...
			if(oplus_chg_show_vooc_logo_ornot() == 1){
				if(chip->support_11v3a_flashcharger_display){
					chip->charger_techonology = oplus_vooc_convert_fast_chg_type_to_chargertechonology(chip->fast_chg_type);
				}
				else{
					chip->charger_techonology = CHARGER_SUBTYPE_FASTCHG_VOOC;
				}
				vooc_xlog_printk(CHG_LOG_CRTI,"chip->charger_technology = %d\n",chip->charger_techonology);
			}
			else{
				chip->charger_techonology = CHARGER_SUBTYPE_DEFAULT;
			}
        }
	return chip->charger_techonology;
}
