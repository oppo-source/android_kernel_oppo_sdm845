...
static struct oppo_mdrv_chip *g_the_chip = NULL;
static void drv8834_check_motor_type(struct oppo_mdrv_chip *chip);

static void drv8834_parse_dts(struct oppo_mdrv_chip * chip)
{
	...
	chip->m1_gpio = of_get_named_gpio(np, "qcom,m1-gpio", 0);
	...
	chip->m0_gpio = of_get_named_gpio(np, "qcom,m0-gpio", 0);
...
static int drv8834_init_pwm_config(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_init_boost(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_init_sleep_gpio(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_init_dir_gpio(struct oppo_mdrv_chip * chip)
{
	int ret = 0;
...
static int drv8834_set_working_mode (int mode)
{
	...
	case MOTOR_MODE_FULL:
		...
		gpio_set_value(g_the_chip->vref_gpio, 0);
	...
	MOTOR_ERR("config change %d %d %d %d %d %d\n",
		...
		gpio_get_value(g_the_chip->vref_gpio),
...
static void drv8834_check_motor_type(struct oppo_mdrv_chip *chip)
{
//    if (is_project(OPPO_17107) && get_PCB_Version() == HW_VERSION__12) { // fi 6 motor
    if (0) { // fi 6 motor
...
static int  drv8834_get_all_config(int* config ,int count)
{
	...
	config[4] = gpio_get_value(g_the_chip->vref_gpio);
	config[5] = gpio_get_value(g_the_chip->dir_gpio);
