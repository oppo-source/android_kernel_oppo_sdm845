...
static struct oppo_dhall_chip *g_the_chip[DHALL_NUM] = {NULL,NULL};
...
static int m1120_get_data(unsigned int id, short *data)
{
	...
	if ((id >= DHALL_NUM) || (g_the_chip[id] == NULL)) {
		MOTOR_LOG("g_the_chip NULL \n");
		return -EINVAL;
	}

	// (1) read data
	err = m1120_i2c_read_block(g_the_chip[id], M1120_REG_ST1, buf, sizeof(buf));
	if (err < 0) {
		MOTOR_LOG(" fail %d \n",err);
		return err;
	}
	...
	// (2) collect data
	if (buf[0] & 0x01) {
		value = m1120_2byte_to_short(g_the_chip[id], buf[2], buf[1]);
	} else {
...
static void m1120_dump_reg(unsigned int id, u8* buf)
{
	...
	for (i = 0; i <= 0x12; i++) {
		...
		err = m1120_i2c_read_block(g_the_chip[id], i, &val, 1);
		if (err < 0) {
...
static int m1120_set_reg(unsigned int id, int reg, int val)
{
	...
	m1120_i2c_write_block(g_the_chip[id], (u8)reg, &data,1);
...
static bool m1120_is_power_on(unsigned int id)
{
	...
	return g_the_chip[id]->is_power_on;
}
...
static int m1120_set_power(struct oppo_dhall_chip *chip, bool on)
{
	...
	if (IS_ERR(chip->power_2v8)) {
		MOTOR_ERR("vdd_2v8 invalid\n");
		return -EINVAL;
	}

	if (IS_ERR(chip->power_1v8)) {
...
static bool m1120_update_threshold(unsigned int id, int position, short lowthd, short highthd)
{
	...
	MOTOR_LOG("id %d ,low:%d, high:%d, position:%d \n",id ,lowthd, highthd, position);

	err = m1120_clear_interrupt(g_the_chip[id]);

	if (g_the_chip[id]->reg.map.intsrs & M1120_VAL_INTSRS_INTTYPE_WITHIN) {

		m1120_short_to_2byte(g_the_chip[id], highthd, &hthh, &hthl);
		m1120_short_to_2byte(g_the_chip[id], lowthd, &lthh, &lthl);

		err |= m1120_i2c_write_block(g_the_chip[id], M1120_REG_HTHH,&hthh, 1);
		err |= m1120_i2c_write_block(g_the_chip[id], M1120_REG_HTHL,&hthl, 1);
		err |= m1120_i2c_write_block(g_the_chip[id], M1120_REG_LTHH,&lthh, 1);
		err |= m1120_i2c_write_block(g_the_chip[id], M1120_REG_LTHL,&lthl, 1);
	}
...
static irqreturn_t m1120_irq0_handler(int irq, void *dev_id)
{
	...
	if (!g_the_chip[DHALL_0]) {
		MOTOR_LOG("g_the_chip NULL \n");
		return -EINVAL;
	}

	disable_irq_nosync(g_the_chip[DHALL_0]->irq);
...
static irqreturn_t m1120_irq1_handler(int irq, void *dev_id)
{
	...
	if (!g_the_chip[DHALL_1]) {
		MOTOR_LOG("g_the_chip NULL \n");
		return -EINVAL;
	}

	disable_irq_nosync(g_the_chip[DHALL_1]->irq);
...
static int m1120_set_detection_mode(unsigned int id, u8 mode)
{
	...
	if(mode & DETECTION_MODE_INTERRUPT) { //interrupt mode
		if (!g_the_chip[id]->irq_enabled) {
			data = g_the_chip[id]->reg.map.intsrs | M1120_DETECTION_MODE_INTERRUPT;

			err = m1120_i2c_write_block(g_the_chip[id], M1120_REG_INTSRS, &data, 1);
			if (err < 0) {
			...
			/* requst irq */
			if (id == 0) {
				if (request_irq(g_the_chip[id]->irq, &m1120_irq0_handler, IRQ_TYPE_LEVEL_LOW, "m1120_0",(void *)g_the_chip[id]->client)) {
					MOTOR_ERR("IRQ LINE NOT AVAILABLE!!\n");
					return -EINVAL;
				}
			} else if (id == 1) {
				if (request_irq(g_the_chip[id]->irq, &m1120_irq1_handler, IRQ_TYPE_LEVEL_LOW, "m1120_1",(void *)g_the_chip[id]->client)) {
					MOTOR_ERR("IRQ LINE NOT AVAILABLE!!\n");
					return -EINVAL;
				}
			} else {
			...
			irq_set_irq_wake(g_the_chip[id]->irq, 1);

			g_the_chip[id]->irq_enabled = 1;
		}
	} else { // polling mode
		if (g_the_chip[id]->irq_enabled) {
			data = g_the_chip[id]->reg.map.intsrs & (0xFF - M1120_DETECTION_MODE_INTERRUPT);
			...
			disable_irq(g_the_chip[id]->irq);
			free_irq(g_the_chip[id]->irq, NULL);

			g_the_chip[id]->irq_enabled = 0;
		}
	}
...
static int m1120_enable_irq(unsigned int id, bool enable)
{
	...
	if (enable) {
		enable_irq(g_the_chip[id]->irq);
	} else {
		disable_irq_nosync(g_the_chip[id]->irq);
...
static int m1120_clear_irq(unsigned int id)
{
	...
	m1120_clear_interrupt(g_the_chip[id]);
...
static int m1120_get_irq_state(unsigned int id)
{
	...
	return ((g_the_chip[id]->reg.map.intsrs & M1120_DETECTION_MODE_INTERRUPT) ? 1 : 0);
}
...
static int m1120_reset_device(struct oppo_dhall_chip *chip)
{
	...
	chip->reg.map.intsrs = M1120_DETECTION_MODE | M1120_SENSITIVITY_TYPE;
...
static void m1120_parse_dts(struct oppo_dhall_chip * chip)
{
	...
	chip->irq_state = pinctrl_lookup_state(chip->pctrl, "hall0_interrupt_input");
	if (IS_ERR(chip->irq_state)) {
		chip->irq_state = pinctrl_lookup_state(chip->pctrl, "hall1_interrupt_input");
		if (IS_ERR(chip->irq_state)) {
...
static int m1120_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		MOTOR_ERR("m1120_setup_eint fail \n");
		goto fail;
	}
