...
int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info *panel_data)
{
    ...
    if (is_project(OPPO_17107) || is_project(OPPO_17127) || is_project(OPPO_17109)) {
        panel_data->tp_type = TP_SAMSUNG;
		if(is_project(OPPO_17109))
			prj_id = OPPO_17107;
		else
			prj_id = OPPO_17107;
    }
...
int preconfig_power_control(struct touchpanel_data *ts)
{
    ...
    if (is_project(OPPO_17107) || is_project(OPPO_17127)) {
        if ((is_project(OPPO_17107) && ((pcb_verison < HW_VERSION__13) || (pcb_verison == HW_VERSION__14))) || (is_project(OPPO_17127) && (pcb_verison == HW_VERSION__10))) {
        } else {
